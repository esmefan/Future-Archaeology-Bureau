...
function initEventListeners() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (e.target.classList.contains('submit-btn') || e.target.closest('.submit-btn')) {
            ...
            if (relicData) {
                window.mockRelics.unshift(relicData);
                switchView('feed');
...
function populateCategorySelectors() {
    ...
    // 设置默认值
    category1Select.value = window.categories[0];
    category2Select.value = window.categories[1];
}
...
function renderFeed() {
    ...
    window.mockRelics.forEach(relic => {
...
function createRelicCard(relic) {
    ...
    const accentColor = window.sourceAccentColor[relic.source.type];
    const tagStyle = window.getTagStyle(relic.categories[0] || '');
    const sourceBadge = window.getSourceBadgeStyle(relic.source.type);
